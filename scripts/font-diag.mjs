...
(async()=>{
  ...
  await fill(p,"diag"+Date.now().toString(36).slice(-4)); await tap(p,"Get started"); await sleep(6000); await tap(p,"Continue to app"); await sleep(8000);
  const info=await p.evaluate(()=>{
    const fonts=[...document.fonts].map(f=>f.family+":"+f.status);
    // find an icon glyph element (the nav uses Text with fontFamily Ionicons)
    const all=[...document.querySelectorAll("*")];
    const iconish=all.filter(n=>{const cs=getComputedStyle(n);return /ionicon/i.test(cs.fontFamily||"")}).slice(0,3)
      .map(n=>({ff:getComputedStyle(n).fontFamily, txt:[...(n.textContent||"")].map(c=>c.codePointAt(0).toString(16)).join(","), w:n.getBoundingClientRect().width}));
    return {fonts:[...new Set(fonts)], iconCount:iconish.length, iconish};
  });
