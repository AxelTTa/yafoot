...
(async()=>{
  ...
  await fill(p,"back"+Date.now().toString(36).slice(-4)); await tap(p,"Get started"); await sleep(6000); await tap(p,"Continue to app"); await sleep(7000);
  // open a match detail (Upcoming) and screenshot to confirm back arrow
  await tap(p,"Upcoming"); await sleep(2500);
  await tapHas(p,"Tap to predict"); await sleep(3500);
  await p.screenshot({path:"/tmp/back-match.png"});
  // settings via profile gear
  await tap(p,"Profile"); await sleep(3000);
  await p.screenshot({path:"/tmp/back-profile.png"});
  console.log("done");
  await b.close();
